...
const authenticate = (email, pass, fn) => {
  ...
  db.query("CALL login_student(?)", [email], (err, result) => {
    if (err) throw err;
    if (!result[0][0]) return fn(null, null);
    let student = result[0][0];
    hash(
      { password: pass, salt: student.salt },
...
router.delete("/api/delete_student", function (req, res) {
  ...
  db.query("CALL delete_student(?)", [studentId], (err, result) => {
    if (err) {
...
router.patch("/api/update_email", function (req, res) {
  ...
  db.query("CALL update_email(?,?)", [studentId, email], (err, result) => {
    if (err) {
